...
def package_area(package: Package) -> int:
...
def container_loading(groupage: Groupage, available_containers_id: set[str]) -> Tuple[set[str], list]:
    ...
    # Container dimensions in cm (rounded low)
    width, length, height = 235, 589, 239 # Standart (not fit for EU palet)
    container_area: int = width*length

    for shipment in groupage.set_of_shipments:
        shipment_minimal_area = sum([package_area(package) for package in shipment.set_of_packages])
        if shipment_minimal_area >= 0.76*container_area:
            # we are sure that the shipment will fit in more than one container
            pass
