...
class TestSetOfShipments(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.shipment2 = OutBoundShipment(departure_date=datetime.datetime(2022, 5, 12, 16, 34), expected_arrival_date=datetime.datetime(
            2022, 5, 11, 16, 34), status="left", set_of_packages=self.database2, sender="EntrepotNostos1", adressee="Jo")
        self.first_shipment_id = "IS" + str(next(shipments_ids) - 2)
    ...
    def test_remove(self):
        ...
        with self.assertRaises(KeyError) as err:
            self.set_of_shipments.remove("IS"+str(next(shipments_ids)))
        self.assertEqual(err.exception.args[0], "This id does not exist")
